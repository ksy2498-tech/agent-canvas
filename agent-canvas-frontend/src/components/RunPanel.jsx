...
export default function RunPanel() {
  ...
  const handlePayload = (payload) => {
    ...
    if (payload.type === 'message') setRunMessages([...(useGraphStore.getState().runMessages || []), { role: 'assistant', text: payload.text }]);
    if (payload.type === 'error') {
      const message = payload.message || 'Run failed';
      setRunMessages([...(useGraphStore.getState().runMessages || []), { role: 'error', text: message }]);
    ...
    if (payload.type === 'done') {
      ...
      setPausedRunId(null);
      setRunningNodeId(null);
      if (payload.output) setRunMessages([...(useGraphStore.getState().runMessages || []), { role: 'assistant', text: payload.output }]);
  ...
  const send = async () => {
    ...
    setRunMessages([...(useGraphStore.getState().runMessages || []), { role: 'user', text: runQuery }]);
    setRunHistory([{ id: runId, query: runQuery, status: 'running', startedAt: new Date().toISOString(), events: [] }, ...(useGraphStore.getState().runHistory || [])]);
    setSelectedStateEvent(null);
    setPausedRunId(null);
    setRunStatus('running');
  ...
  return (
    <>
      ...
      {selectedStateEvent ? <button type="button" aria-label="Close snapshot" className="fixed bottom-0 left-0 right-[840px] top-0 z-[35] cursor-default bg-transparent" onClick={() => setSelectedStateEvent(null)} /> : null}
...
function StateDrawer({ event, onClose }) {
  ...
  const runtimePreview = event.runtime_preview;
  return <aside className="fixed right-[420px] top-0 z-40 h-full w-[420px] overflow-y-auto border-l border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-slate-900"><div className="sticky top-0 z-10 border-b border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center justify-between"><div><h2 className="font-semibold">Snapshot</h2><p className="text-xs text-slate-500">{event.label || event.nodeId || event.type}</p></div><button className="icon-btn" onClick={onClose}><X size={17} /></button></div><div className="mt-3 grid grid-cols-2 rounded-md border border-slate-200 bg-slate-50 p-1 text-xs dark:border-slate-700 dark:bg-slate-950"><button type="button" onClick={() => setTab('state')} className={`rounded px-2 py-1.5 font-medium ${tab === 'state' ? 'bg-white text-blue-700 shadow-sm dark:bg-slate-800 dark:text-blue-300' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}>State</button><button type="button" onClick={() => setTab('runtime')} className={`rounded px-2 py-1.5 font-medium ${tab === 'runtime' ? 'bg-white text-teal-700 shadow-sm dark:bg-slate-800 dark:text-teal-300' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}>Runtime</button></div></div><div className="space-y-2 p-4">{tab === 'runtime' ? (runtime ? Object.entries(runtime).map(([key, value]) => <details key={key} className="rounded-md border border-teal-200 p-2 dark:border-teal-900" open={['session', 'tool_results', 'nlp', 'scratch'].includes(key)}><summary className="cursor-pointer text-xs font-medium">{key}</summary><pre className="mt-2 max-h-64 overflow-auto whitespace-pre-wrap break-words text-[11px]">{JSON.stringify(value, null, 2)}</pre></details>) : runtimePreview ? <pre className="rounded-md border border-teal-200 p-2 text-[11px] dark:border-teal-900">{JSON.stringify(runtimePreview, null, 2)}</pre> : <p className="text-xs text-slate-500">No runtime snapshot for this event.</p>) : (state ? Object.entries(state).map(([key, value]) => <details key={key} className="rounded-md border border-slate-200 p-2 dark:border-slate-700" open={['current_output', 'node_results', 'loaded_session'].includes(key)}><summary className="cursor-pointer text-xs font-medium">{key}</summary><pre className="mt-2 max-h-64 overflow-auto whitespace-pre-wrap break-words text-[11px]">{JSON.stringify(value, null, 2)}</pre></details>) : <p className="text-xs text-slate-500">No state snapshot for this event.</p>)}</div></aside>;
}
