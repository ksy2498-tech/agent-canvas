...
const keys = ['query', 'messages', 'current_output', 'metadata', 'nlp_result'];
export default function SessionSavePanel({ node }) {
  const aliases = useDbAliases();
  const updateNodeData = useGraphStore((s) => s.updateNodeData);
  const data = { id: node.id, sessionStore: 'Local SQLite', path: './sessions.db', keysToSave: ['messages'], mode: 'overwrite', ...node.data };
