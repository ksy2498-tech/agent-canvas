...
export default function NLPNodePanel({ node }) {
  ...
  const data = { id: node.id, engine: 'Kiwi', analysisType: 'morpheme', inputKey: 'current_output', outputKey: 'nlp_result', resultTarget: 'runtime', summaryKey: 'metadata.nlp_summary', ...node.data };
  ...
  return <BaseFields data={data} setData={setData} mcpModes={['tool-only']}>
    ...
    <Field label="Result target"><SelectInput value={data.resultTarget} onChange={(resultTarget) => setData({ resultTarget })} options={['runtime', 'state', 'both']} /></Field>
    <Field label="Output key"><TextInput value={data.outputKey} onChange={(outputKey) => setData({ outputKey })} /></Field>
    {data.resultTarget === 'runtime' ? <Field label="State summary key"><TextInput value={data.summaryKey} onChange={(summaryKey) => setData({ summaryKey })} placeholder="metadata.nlp_summary" /></Field> : null}
  </BaseFields>;
}
